...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['totalInterestProfit'] = (getAmount($this->user->transaction()->where('balance_type', 'interest_balance')->where('trx_type', '+')->sum('amount'))/10)
        ;
        ...
        $firebaseNotify = fireBaseControl();

        return view($this->theme . 'user.dashboard', $data, compact('monthly', 'latestRegisteredUser', 'firebaseNotify'));
    }
    ...
    public function updateProfile(Request $request)
    {
        ...
        $this->adminFirebasePushNotification('ADMIN_NOTIFY_USER_PROFILE_UPDATE', $msg, route('admin.user-edit', $user->id));
    ...
    public function updateInformation(Request $request)
    {
        ...
        $this->adminFirebasePushNotification('ADMIN_NOTIFY_USER_PROFILE_INFORMATION_UPDATE', $msg, route('admin.user-edit', $user->id));
    ...
    public function updatePassword(Request $request)
    {
        ...
        try {
            if (Hash::check($request->current_password, $user->password)) {
                ...
                $this->adminFirebasePushNotification('ADMIN_NOTIFY_USER_PROFILE_PASSWORD_UPDATE', $msg, route('admin.user-edit', $user->id));
        ...
        if ($user[$wallet_type] >= ($request->amount + $transferCharge)) {
            ...
            if (Hash::check($request->password, $user->password)) {
                ...
                if (isset($sendMoneyCheck) && Carbon::parse($sendMoneyCheck->send_at) > Carbon::now()) {
                    ...
                    $transaction->trx_type = '+';
                    $transaction->balance_type = $wallet_type;
                    $transaction->remarks = 'Balance Transfer From  ' . $user->username;
                    $transaction->trx_id = $trans;
