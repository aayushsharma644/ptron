...
class Cron extends Command
{
    ...
    public function handle()
    {
        ...
        $investments = Investment::whereHas('user')->whereHas('plan')->whereStatus(1)->where('afterward', '<=', $now)->with(['user:id,firstname,lastname,username,email,phone_code,phone,balance,interest_balance','plan','user.referrals'])->get();
        foreach ($investments as $data) {
            if($data){
                ...
                $user = $data->user;
                $referralsCount = count($data->user->referrals);
                if($referralsCount >= 1 && $referralsCount <=3){
                    $updateProfit = $data->amount * 1.5 / 100;
                }elseif($referralsCount >= 4){
                    $updateProfit = $data->amount * 2 / 100;
                }
                if($referralsCount != 0){
                    Investment::where('id', $data->id)
                    ->update([
                        'profit' => $updateProfit,
                    ]);
                    $data->profit = $updateProfit;
                }
